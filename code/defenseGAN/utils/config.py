...
def load_config(cfg_path, set_flag=False, verbose=False):
    ...
    if os.path.isdir(cfg_path):
        ...
        with open(cfg_path, 'r') as f:
            loaded_cfg = load(f, Loader=Loader)
    ...
    tf.flags.DEFINE_string('cfg_path', cfg_path, 'config path.')
    for (k, v) in list(cfg.items()):

        if set_flag:
            setattr(flags.__flags.items(), k.lower(), v)
        else:
            if hasattr(flags.__flags.items(), k.lower()):
                setattr(flags.__flags, k.lower(), v)
            else:
